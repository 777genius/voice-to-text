...
use std::{ffi::{c_void, CStr}, sync::mpsc, time::{Duration, Instant}};
...
#[repr(C)]
struct Extended { times: [u64; 2], scheduling: [i32; 8], name: [i8; 64] }
struct Threads { ports: *mut u32, count: u32 }
impl Drop for Threads {
    fn drop(&mut self) { unsafe {
        for i in 0..self.count { mach_port_deallocate(mach_task_self_, *self.ports.add(i as usize)); }
        vm_deallocate(mach_task_self_, self.ports as usize, self.count as usize * 4);
    } }
}
pub fn threads() -> Result<(u64, u32)> { unsafe {
    let mut sample = Threads { ports: std::ptr::null_mut(), count: 0 };
    ensure!(task_threads(mach_task_self_, &mut sample.ports, &mut sample.count) == 0, "thread enumeration");
    let mut ids = Vec::new();
    for i in 0..sample.count {
        let port = *sample.ports.add(i as usize);
        let mut info = Extended { times: [0; 2], scheduling: [0; 8], name: [0; 64] };
        ...
        if thread_info(port, 5, (&mut info as *mut Extended).cast(), &mut count) != 0 { continue; }
        ensure!(count == 28 && info.name.contains(&0), "thread metadata");
        if CStr::from_ptr(info.name.as_ptr()).to_bytes() == b"continuation-native" {
            let mut info = [0u64; 3];
            let mut count = 6;
            ensure!(thread_info(port, 4, info.as_mut_ptr().cast(), &mut count) == 0 && count == 6, "thread identity");
            ids.push(info[0]);
        }
    }
...
impl Probe {
    pub fn new(pid: i32) -> Self {
        ...
        std::thread::spawn(move || unsafe {
            ...
            let pool: id = msg_send![class!(NSAutoreleasePool), new];
            {
                ...
                let key = Owned(CFStringCreateWithBytes(std::ptr::null(), key.as_ptr(), key.len() as isize, 0x08000100, false));
                for reply in rx {
                    let result = (|| -> Result<_> {
                        ensure!(!app.0.is_null() && !key.0.is_null(), "probe allocation");
                        ensure!(CFGetTypeID(app.0) == AXUIElementGetTypeID(), "application type");
                        let mut actual = 0;
                        ensure!(AXUIElementGetPid(app.0, &mut actual) == 0 && actual == pid, "application pid");
                        ensure!(AXUIElementSetMessagingTimeout(app.0, 0.1) == 0, "probe budget");
                        ...
                        if error == 0 {
                            ensure!(!value.0.is_null() && CFGetTypeID(value.0) == AXUIElementGetTypeID(), "focused type");
                            ensure!(AXUIElementGetPid(value.0, &mut actual) == 0 && actual == pid, "focused pid");
                        }
                        Ok((error, elapsed))
                    })();
    ...
    pub async fn read(&self) -> Result<(i32, Duration)> {
        ...
        self.0.send(tx).map_err(|_| anyhow::anyhow!("probe closed"))?;
